...
class ReplayMemory_1d:
    ...
    def __init__(self,
        ...
        Args
         max_size: int
         window_size: int
            the number of 1d-data for state
        input_shape: int
            the length of 1d-data

        """

        self._max_size = max_size
        self._num_data = num_data
        self._length = input_shape
        self._memory = []
    ...
    def sample(self, batch_size, indexes=None):
        ...
        samples = random.sample(self._memory, min(batch_size, len(self._memory)))
        zipped = list(zip(*samples))
        zipped[0] = np.reshape(zipped[0], (-1, self._length, self._num_data))
        zipped[3] = np.reshape(zipped[3], (-1, self._length, self._num_data))
        return zipped
...
class ReplayMemory:
    ...
    def sample(self, batch_size, indexes=None):
        ...
        samples = random.sample(self._memory, min(batch_size, len(self._memory)))
        zipped = list(zip(*samples))
        zipped[0] = np.reshape(zipped[0], (-1, self._input_size))
        zipped[3] = np.reshape(zipped[3], (-1, self._input_size))
        return zipped
